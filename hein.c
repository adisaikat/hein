...
struct editorConfig {
  ...
  // TODO: (raw and inset modes universal var)
};
...
int editorRowCxToRx(erow *row, int cx) {
  ...
  for (j = 0; j < cx; j++) {
    if (row->chars[j] == '\t') {
      rx += (HEIN_TAB_STOP - 1) - (rx % HEIN_TAB_STOP);
    }
    rx++;
  }
  return rx;
...
void editorSave() {
  ...
  free(buf);
  editorSetStatusMessage("Error saving the file!");
  E.dirty = 0;
}
...
void editorMoveCursor(int key) {
  ...
  if (row && nxt == 0) {
    E.cx = (E.pos > row->size) ? row->size : E.pos;
  } else if (row && nxt == 1) {
    E.pos = row->size;
    E.cx = E.pos;
  } else {
...
void editorDrawStatusBar(struct abuf *ab) {
  ...
  int rlen = snprintf(rstatus, sizeof(rstatus), "%d/%d",
                      E.numrows == 0 ? 0 : E.cy + 1, E.numrows);
  if (len > E.screencols) {
    len = E.screencols;
  }
